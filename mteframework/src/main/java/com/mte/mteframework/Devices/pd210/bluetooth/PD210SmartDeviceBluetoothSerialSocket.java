...
public class PD210SmartDeviceBluetoothSerialSocket extends BluetoothGattCallback implements Runnable
{
    //Credit to Kai Morich
    //############################################################################################
    //############################################################################################
    /**
    ...
    //private PD210SmartDeviceBluetoothSerialListener_t listener;
    ...
    private BroadcastReceiver pairingBroadcastReceiver = null;
    private final BroadcastReceiver disconnectBroadcastReceiver;
    private BluetoothGatt gatt;
    private BluetoothGattCharacteristic readCharacteristic, writeCharacteristic;
    ...
    private PD210SmartDeviceBluetooth.BluetoothDeviceType deviceType = PD210SmartDeviceBluetooth.BluetoothDeviceType.BLUETOOTH_CLASSIC;
    ...
    //##################################################################################################################################################################
    //##################################################################################################################################################################
    public PD210SmartDeviceBluetoothSerialSocket(Context context, BluetoothDevice device, MTEBluetoothDeviceItem itemdevice)
    {
        deviceItem = itemdevice;
        deviceType = itemdevice.getBluetoothType();
        if(itemdevice.getBluetoothType() == PD210SmartDeviceBluetooth.BluetoothDeviceType.BLUETOOTH_CLASSIC)
        {
            writeBuffer = new ArrayList<>();
            pairingIntentFilter = new IntentFilter();
    ...
    void connect(PD210SmartDeviceBluetoothSerialListener listener) throws IOException
    {
        if(deviceType == PD210SmartDeviceBluetooth.BluetoothDeviceType.BLUETOOTH_CLASSIC)
        {
            this.listener = listener;
            context.registerReceiver(disconnectBroadcastReceiver, new IntentFilter("com.mte.mte_sia.disconnect"));
            ...
            canceled = false;
            this.listener = listener;
            context.registerReceiver(disconnectBroadcastReceiver, new IntentFilter(PD210BluetoothConstants.INTENT_ACTION_DISCONNECT));
        ...
        if(deviceType== PD210SmartDeviceBluetooth.BluetoothDeviceType.BLUETOOTH_CLASSIC)
        {
            ...
            synchronized (writeBuffer) {
                if(len <= payloadSize)
                {
                    ...
                    data0 =  Arrays.copyOf(data,len);//data;
                } else {
                    data0 = Arrays.copyOfRange(data, 0, payloadSize);
                }
                if(!writePending && writeBuffer.isEmpty() && delegate.canWrite()) {
                ...
                if(len > payloadSize)
                {
                    for(int i=1; i<(len+payloadSize-1)/payloadSize; i++) {
                        int from = i*payloadSize;
                        int to = Math.min(from+payloadSize, len);
                        writeBuffer.add(Arrays.copyOfRange(data, from, to));
                        Log.d(TAG,"write queued, len="+(to-from));
                    }
                }
